...
var ImgZipper_compress = function compress(img, scale, quality, type, callback, orientation, width, height, callbackParameter) {
  ...
  imgcar.onload = function () {
    ...
    oh *= scale;
    ow *= scale;
    var canvasoption = new map_default.a();
    canvasoption.set(1, {
      width: ow,
      height: oh,
      angel: 0
    ...
    var Canvasoption = canvasoption.get(direction);
...
var ImgZipper_ImgZipper = function ImgZipper(file, Callback, option) {
  if (file) {
    var Scale = parse_float_default()(option.scale) || 1;
    var Quality = parse_float_default()(option.quality) || .82;
    var Type = option.type || 'image/jpeg';
    var Exif = option.exif == false ? false : true;
    var Width = option.width || false;
    var Height = option.height || false;
    var CallbackParameter = option.callbackParameter || null;
    ...
    if (Exif) {
      ...
      exifreader.onload = function (E) {
        ...
        reader.onload = function (e) {
          ImgZipper_compress(e.target.result, Scale, Quality, Type, Callback, Orientation, Width, Height, CallbackParameter);
        };
      };
    } else {
      reader.readAsDataURL(file);
      ...
      reader.onload = function (e) {
        ImgZipper_compress(e.target.result, Scale, Quality, Type, Callback, false, Width, Height, CallbackParameter);
      };
    }
  }
